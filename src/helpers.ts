...
export function makeCanvasPannable(engine: Engine, svgContainer: SVGSVGElement) {
    ...
    svgContainer.addEventListener("mousemove", (e: MouseEvent) => {
        if (engine.isPanning) {
            const pointerDelta = {
                x: engine.lastPointerPosition.x
                    ? e.clientX - engine.lastPointerPosition.x
                    : 0,
                y: engine.lastPointerPosition.y
                    ? e.clientY - engine.lastPointerPosition.y
                    : 0,
            };
            engine.lastPointerPosition = {
                x: e.clientX,
            ...
            const { x, y, width, height } = engine.Svg.viewbox();
